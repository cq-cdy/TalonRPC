...
int main() {
    ...
    struct sockaddr_in serverAddr{}, controlAddr{}, clientAddr{};
    ...
    while (true) {
        ...
        for (int i = 0; i < numEvents; ++i) {
            if (events[i].data.fd == serverSocket) {
                clientSocket =
                    ::accept(serverSocket, reinterpret_cast<sockaddr *>(&controlAddr), &serverAddrLen);
                printf("c = %d \n",clientSocket);
                if (clientSocket < 0) {
                ...
                if (bytesRead <= 0) {
                    ...
                    if (command == "add") {
                        // todo  Handle adding a new service entry to serviceMap
                    } else if (command == "delete") {
                        // todo Handle deleting a service entry from serviceMap
                    } else if (command == "lookup") {
                        // todo Handle looking up a service entry in serviceMap
                    } else {
                ...
                size_t byteRead = recv(events[i].data.fd, buffer, sizeof(buffer) - 1, 0); // 为'\0'留下一个位置
