...
namespace talon {
    ...
    void CoredumpHandler(int signal_no) {
        ...
        g_logger->flush();
        pthread_join(g_logger->getAsyncLopger()->m_thread, nullptr);
        pthread_join(g_logger->getAsyncAppLopger()->m_thread, nullptr);
    ...
    std::set<std::shared_ptr<AsyncLogger>> t_set;
    Logger::Logger(LogLevel level, int type /*=1*/) : m_set_level(level), m_type(type) {
        ...
        m_asnyc_app_logger = std::make_shared<AsyncLogger>(
                ...
                Config::GetGlobalConfig()->m_log_file_path,
                Config::GetGlobalConfig()->m_log_max_file_size);
        t_set.insert(m_asnyc_logger);
        t_set.insert(m_asnyc_app_logger);
    }
    ...
    AsyncLogger::AsyncLogger(std::string  file_name, std::string  file_path, int max_size)
            : m_file_name(std::move(file_name)), m_file_path(std::move(file_path)), m_max_file_size(max_size) {
    ...
    void* AsyncLogger::Loop(void* arg) {
        ...
        while(true) {
            ...
            if (logger->m_file_hanlder == nullptr) {
                logger->m_reopen_flag = true;
            }
