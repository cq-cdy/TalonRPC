...
void RpcChannel::CallMethod(const google::protobuf::MethodDescriptor *method,
    ...
    m_client->connect([&]() mutable {
        auto my_controller =
            dynamic_cast<talon::RpcController *>(channel->getcontroller());
                ...
                channel->getTcpClient()->readMessage(
                    ...
                    [&](
                        ...
                        std::shared_ptr<talon::TinyPBProtocol> rsp_protocol =
                            std::dynamic_pointer_cast<talon::TinyPBProtocol>(
                                msg);
                        ...
                        if (my_controller == nullptr) {
                            ERRORLOG(
                                "failed callmethod,RpcController convert error")
                            return;
                        }
                        ...
                        DEBUGLOG("rsp_protocol->m_pb_data =  %s ",
                                 rsp_protocol->m_pb_data.c_str());
                        if (rsp_protocol->m_err_code != 0) {
                            ERRORLOG(
                                "%s | call rpc methood[%s] failed,error "
                                "code[%d],error "
                                "info[%s]",
                                rsp_protocol->m_msg_id.c_str(),
                                rsp_protocol->m_method_name.c_str(),
