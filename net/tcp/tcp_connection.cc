...
#include "log.h"
namespace talon {
    TcpConnection::TcpConnection(IOThread *io_thread, int fd, int buffer_size, const NetAddr::s_ptr& peer_addr)
    :m_io_thread(io_thread), m_peer_addr(peer_addr), m_state(TcpState::NotConnected), m_fd(fd){
        ...
        // 把fd和与客户端的write、read回调函数封装到 fd_event 并挂在所属子线程中的epoll红黑树上
