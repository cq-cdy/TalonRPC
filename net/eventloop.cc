...
#define ADD_TO_EPOLL() \
    ...
    if((epoll_ctl(m_epoll_fd,op,event->getFd(),&tmp)) < 0){\
        ERRORLOG("failed epoll_ctl when add fd,errno = %d ,err = %s",errno,strerror(errno));\
    }\
...
namespace talon {
    ...
    Eventloop::Eventloop() {
        ...
        if (m_epoll_fd < 0) {
            ERRORLOG("failed to create event loop，epoll create failed err info[%d]", errno);
            exit(Error);
        }
        if ((m_wakeup_fd = eventfd(0, EFD_NONBLOCK)) < 0) {
            ERRORLOG("failed to create event loop，m_wakeup_fd create failed err info[%d]", errno);
        }
        epoll_event event{};
        ...
        if (epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, m_wakeup_fd, &event) < 0) {
            ERRORLOG("failed to create event loop， eventfd add err info[%d]", errno);
        }
        initWakeUpFdEevent();
        t_current_eventloop = this;
        INFOLOG("succeed create event loop in thread:%d", m_thread_id);
    }
...
int main() {
    ...
    // Config 和  Logger 强关联性，因此，一定要先
    talon::Config::SetGlobalConfig("config.xml");
    talon::Logger::InitGlobalLogger();

    talon::Eventloop eventloop;
    return 0;
}